...
namespace CorralesEventos.Utilerias
{
    public class dgvExporta
    {
        ...
        //Constructor
        public dgvExporta(DataGridView aDataGridView, PrintDocument aPrintDocument, bool CenterOnPage, bool WithTitle, string aTitleText, Font aTitleFont, Color aTitleColor, bool WithPaging)
        {
            try
            {
        ...
        private void Calculate(Graphics g, float fs)
        {
            try
            {
                if (PageNumber == 0) // Just calculate once
                {
                    SizeF tmpSize = new SizeF(); Font tmpFont; float tmpWidth;

                    TheDataGridViewWidth = 0;
                    for (int i = 0; i < TheDataGridView.Columns.Count; i++)
                    {
                        tmpFont = new Font("Arial", fs); // TheDataGridView.ColumnHeadersDefaultCellStyle.Font;
        ...
        private void DrawHeader(Graphics g, float fs)
        {
            try
            {
                ...
                if (IsWithPaging)
                {
                    ...
                    Font PageStringFont = new Font("Tahoma", 10, FontStyle.Regular, GraphicsUnit.Document);
                ...
                // Setting the HeaderFont style (tamaño de la Letra)
                Font HeaderFont = new Font("Arial", fs); // TheDataGridView.ColumnHeadersDefaultCellStyle.Font;
                ...
                for (int i = (int)mColumnPoints[mColumnPoint].GetValue(0); i < (int)mColumnPoints[mColumnPoint].GetValue(1); i++)
                {
                    ...
                    //if(TheDataGridView.Columns[i].Name == "direccion")
                    //    ColumnWidth = 95.0f;
                    //else
        ...
        private bool DrawRows(Graphics g, float fs)
        {
            try
            {
                // Setting the LinePen that will be used to draw lines and rectangles (derived from the GridColor property of the DataGridView control)
                ...
                while (CurrentRow < TheDataGridView.Rows.Count)
                {
                    if (TheDataGridView.Rows[CurrentRow].Visible) // Print the cells of the CurrentRow only if that row is visible
                    {
                        ...
                        RowFont = new Font("Arial", fs); // TheDataGridView.Rows[CurrentRow].DefaultCellStyle.Font;
                        ...
                        for (int CurrentCell = (int)mColumnPoints[mColumnPoint].GetValue(0); CurrentCell < (int)mColumnPoints[mColumnPoint].GetValue(1); CurrentCell++)
                        {
                            ...
                            //if (TheDataGridView.Columns[CurrentCell].Name == "direccion")
                            //    ColumnWidth = 95.0f;
                            //else
        ...
        public bool DrawDataGridView(Graphics g, float fs)
        {
            try
            {
                Calculate(g, fs);
                DrawHeader(g, fs);
                bool bContinue = DrawRows(g, fs);
                return bContinue;
            }
            catch (Exception ex)
            {
                MessageBox.Show("La Operción Fallo: " + ex.Message.ToString(), Application.ProductName + " - Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
    }
